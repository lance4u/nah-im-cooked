...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        for (const guild of client.guilds.cache.values()) {
            ...
            if (member) matchedServers.push(`**${guild.name}** — ${guild.memberCount.toLocaleString()} members`);
        }

        if (matchedServers.length === 0) {
            return interaction.editReply({ content: `**${targetUser.tag}** is not in any server the bot has joined.` });
        }
